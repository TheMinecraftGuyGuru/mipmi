...
import (
	...
	"outband/internal/idrac"
)

func main() {
	host := env("OUTBAND_BMC_HOST", "192.168.9.237")
	...
	insecure := true
	if v := os.Getenv("OUTBAND_IDRAC_INSECURE"); v == "0" || v == "false" || v == "FALSE" {
		insecure = false
	}
	transport := env("OUTBAND_IDRAC_TRANSPORT", "auto")

	a := idrac.New(idrac.Config{
		Host:               host,
		Port:               port,
		User:               user,
		Password:           pass,
